...
3. **Test it.** See `## Testing

1. **Dry run first** — always run with `-WhatIf` before making live changes:
   ```powershell
   .\GroupClone.ps1 -SourceUser source@contoso.com -TargetUser target@contoso.com -WhatIf
   ```
2. Review the generated `GroupClone_Results_*.csv` to confirm the expected groups, ownerships, and mailbox delegations were detected.
3. **Use a non-production tenant** whenever possible. If testing against production, ensure the target user is a test account.
4. After a live run, spot-check a few groups in Entra ID and mailbox permissions in EXO to verify the changes applied correctly.` in the README for how to verify your change locally.
